-- Eigenes Buchungssystem: 500er beim Buchen beheben.
--
-- Ursache: In einer RETURNS TABLE-Funktion sind `cancel_token`, `starts_at`
-- und `ends_at` auch Output-Variablen. Unqualifizierte SELECT/RETURNING-
-- Spalten mit denselben Namen werden dadurch in PL/pgSQL mehrdeutig.
...
BEGIN
  ...
  SELECT ia.id, ia.cancel_token, ia.starts_at, ia.ends_at
    INTO existing_id, existing_token, existing_starts, existing_ends
    FROM public.interview_appointments AS ia
